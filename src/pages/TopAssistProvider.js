...
const TopAssistProvider = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      setShowScroll(window.pageYOffset > 300); // 300px 이상 스크롤 시 버튼 보이기
    };
  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      try {
        const q = query(collection(db, 'players'), orderBy('assists', 'desc'));
